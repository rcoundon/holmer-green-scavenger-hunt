<template>
  <div class="container is-mobile">
    <p>
      Note: out of <b class="has-text-primary">54</b> cards, <b class="has-text-success">{{ allocatedCards }}</b> have currently
      been allocated
    </p>
    <br />
    <p class="is-centered has-text-weight-semibold has-text-primary">
      You have collected {{ totalCards }} {{ cardWord }} and {{ totalCardsCorrect }} {{ successCardWord }} correct
    </p>
    <p v-if="totalCardsCorrect === 54" style="padding-top: 1em" class="has-text-success is-size-4 has-text-weight-bold">
      CONGRATULATIONS!!! You found all the cards. Thanks for taking part
    </p>
    <br />
    <b-field grouped position="is-centered">
      <b-field>
        <b-button type="is-success" @click="addCard">Add a card</b-button>
      </b-field>
      <b-field>
        <b-button type="is-success" @click="checkAnswers">Check answers</b-button>
      </b-field>
    </b-field>
    <b-field style="margin-top: -1em">
      <b-button type="is-success" @click="showStreetCounts">Show numbers for each street</b-button>
    </b-field>
    <div class="separator">
      <joker colour="Red" :street="redJoker" :streets="streets" @save-joker="saveJoker" @delete-joker="deleteJoker" />

      <joker colour="Black" :street="blackJoker" :streets="streets" @saveJoker="saveJoker" @deleteJoker="deleteJoker" />
    </div>
...
export default {
  ...
  computed: {
    ...mapGetters(['cards', 'totalCards', 'totalCardsCorrect', 'redJoker', 'blackJoker']),
    cardWord() {
    ...
    allocatedCards() {
      const totalAllocatedRegularCards = this.config.cardTrail.answers.reduce((acc, curr) => {
      ...
      const totalAllocatedJokers = this.config.cardTrail.jokers.reduce((acc, curr) => {
        if (curr.street !== 'UNALLOCATED') {
          return acc + 1;
        }
        return acc;
  ...
  methods: {
    ...mapActions(['storeCard', 'getCardId', 'storeTotalCardsCorrect', 'deleteCard', 'storeRedJoker', 'storeBlackJoker']),
    getCardWord(total) {
